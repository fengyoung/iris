...
> 当前验证版本：Iris 3.40.0 · 现行持久化遵循 `FileLock` 稳定锁文件与统一原子写约定，详见 [工程可靠性设计](engineering-reliability-design.md)。
...
|:---:|------|:---:|
| 1 | LLM 双通道记忆提取器 | ✅ 已实施 |
| 2 | 会话模式挖掘器 | ✅ 已实施（v3.28.1 修复懒触发） |
| 3 | 全自治生命周期 | ✅ 已实施（v3.28.1 修复裁剪方向） |
...
| Wiki 候选 | 反复讨论的主题 → 自动加入 discover-wiki | "智能巡查" 出现 5+ 次 → 建议创建 Wiki |
