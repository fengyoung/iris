...
游戏服务默认 `127.0.0.1:7862`，限制 16 个同时连接、2 局并行对局、每局 3～12 名玩家。
单请求最多 20 MB，图片最多 2000 万像素，上传暂存区最多 200 MB；暂存图片超过一天后在下一次上传时清理。
已完成会话保留一小时后，在下一次开局时回收；完整复盘与图片保存在 `data/games/`。
SSE 支持 `Last-Event-ID` 续传；事件游标过期会显式提示改看历史复盘。
取消为协作取消，已发送的模型请求需要返回或超时，随后不再安排新请求。

索引出现“已被其他构建更新”时重新构建；模型不兼容时使用 `build-vector-index --force-rebuild`。

黄金集格式：

```json
{"cases":[{"query":"检索问题","expected_chunk_ids":["文档.md::chunk-1"]}]}
```

运行 `python scripts/evaluate_retrieval.py cases.json --project-root . --top-k 5 --min-recall 0.8`。
当前指标的 recall_at_k 为“至少命中一个相关 chunk”的问题命中率，同时检查引用字段完整性和源文件存在性。
该离线门禁验证词法检索与引用基础质量，不替代真实业务问题的人工答案审阅或在线模型效果评测。
