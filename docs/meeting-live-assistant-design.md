...
- **v3.28.1**（当前验证版本）：深度审查批次 1 两项修复——① `suggest_questions` 对 `List[DecisionItem]` 直接 join 抛 TypeError 被静默吞（恰在「存在决策点」这一事件驱动触发条件下建议提问整体失效），改按 CONF_ICON+text 渲染并将 prompt 渲染纳入 try；② TaskReporter 埋点 `data_root` 误传 `_pid_dir.parent`（埋点写 `<root>/tasks/` 而面板读 `<root>/data/tasks/`，会议任务在面板永不可见），改传 `_pid_dir`。回归测试 +4，协议 3.21（不变）
