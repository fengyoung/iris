...
**日期**：2026-08-12 · **最后校验**：2026-08-26 · **状态**：已实现（v3.23.0 落地 → v3.25.0 本地音频 ASR → v3.26.0 四层能力+说话人区分 → v3.26.1 全量优化+评估修复 → v3.26.2 面板双主题视觉方案 → v3.26.3 面板稳定化+并发加固）· **当前验证版本**：产品 3.28.1 / 协议 3.21
...
## 📌 话题 1：质检流程优化（段 1-28）
**讨论**：提出某仓质检流程优化需求，先介绍方案看能否满足
**决策**：✅ 先介绍方案，不满足再联动解决

## ✅ 决策汇总
- 用重点质量指标替代差异指标（speaker_B）
...
- **v3.26.3**（功能基线）：面板稳定化 + 并发加固——① 区域固高布局（语音 3/分析 2/建议提问 2/洞察推送 4 行，不足补空行、超出截断「…」，高度不再跳动）+ alt-screen 进出（保留/恢复终端回滚历史）+ 折行 O(n²)→O(n) + 洞察推送多行渲染 + 长告警/长标题截断（防边框断裂）+ VU emoji 标签（色盲友好）+ 底部累计条 CJK 宽度修正；② 并发加锁——InsightFeed（push/toggle_pause + visible snapshot）、CorrectorAdapter per-speaker 上下文（LRU ≤10 speaker）、AudioCapture buffer（回调↔主线程）；③ 修复——多段批次按 seq 升序落账（原首段最后落账乱序）、analysis_elapsed 实际耗时（原恒 0）、`asr.local.batch_size_s` 真正生效（原硬编码 60）、`doc_rewrite_every` 默认 3；④ 共享常量 CONF_ICON/DECISION_FG 收敛到 models.py + teardown_session_logger（e2e 防句柄泄漏）。测试 187 专项，协议 3.19（不变）
- **v3.28.0**：会议助理行为不变；共享原子写、项目根解析和质量门禁随全项目工程治理统一。
- **v3.28.1**（当前验证版本）：深度审查批次 1 两项修复——① `suggest_questions` 对 `List[DecisionItem]` 直接 join 抛 TypeError 被静默吞（恰在「存在决策点」这一事件驱动触发条件下建议提问整体失效），改按 CONF_ICON+text 渲染并将 prompt 渲染纳入 try；② TaskReporter 埋点 `data_root` 误传 `_pid_dir.parent`（埋点写 `<root>/tasks/` 而面板读 `<root>/data/tasks/`，会议任务在面板永不可见），改传 `_pid_dir`；③ LLM 思考文本污染修复（provider 层全局）——`content` 为空时不再静默回退返回 `reasoning_content`，直接抛 `LLMProviderError`：会议助理各 LLM 调用（段分析/深度校正/建议提问）在思考模型 max_tokens 耗尽等场景下不再产出「思考当分析结果」的污染文本，而是走既有降级链（deadline 超时/熔断路径），行为更可预期。回归测试 +4，协议 3.21（不变）
