...
> 版本：Iris 3.40.0 · 日期：2026-08-26（v3.28.1 基线；v3.35.0 增补 2026-09-10）· 状态：已实现
...
- chunk 构建对完整源快照复用内容 hash，发布前在稳定锁内比对旧摘要指纹；陈旧写者必须重新构建。
- 向量代际读取与发布统一使用 `index.lock`，读者完成后才允许清理旧代际；保存前核对 generation。
- Embedding 批次校验索引、数量、维度和有限数值，禁止缺项错配后继续发布；查询加载校验模型，维度不匹配显式降级。
- LLM 缓存键包含 max_tokens、extra_body 与配置摘要，避免模型/输出约束变更后复用旧答案。
- 游戏 Web 只监听本机，校验 Host/Origin 与上传目录边界，采用有界事件日志和独立订阅游标。
- 全量 3,538 项测试通过，覆盖率 69.63%；新增跨模块回归见 `tests/test_review_regressions.py` 与 `tests/test_games_web.py`。
