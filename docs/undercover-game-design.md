...
- 卧底全出局 → `civilians` 胜；场上卧底人数 **≥** 平民人数 → `spy` 胜。
  判定用**多数即胜**：人数打平当场收局，不进入下一轮。开局约束 `1 ≤ 卧底 < 人数/2` 保证
  平民人数严格多于卧底，卧底须靠投票把差距抹平，而「追平」即为终局。
  与之互补的 `continuing` 判定是「判词为空」，否则会在已判胜负的打平轮**多发**一次
  `round_waiting`，Web 端相位进 `waiting`、步进按钮亮起，玩家为一场已结束的对局点一次步进。
- **判据只此一处**：`_judge_survivors()` 是唯一的判词来源，主循环、断点恢复分支与手动步进的
  继续判定全部经由它得出。三处曾各写一份手写副本，规则一改就要人工同步三处——收敛为单函数后，
  「继续 ⟺ 判词为空」由注释纪律变为结构保证。
- **已知代价：3 人局的博弈纵深为零**。1 名卧底 + 2 名平民时，第 1 轮无论淘汰谁都直达终局
  （淘汰平民 → 1v1 打平判卧底胜；淘汰卧底 → 卧底清零判平民胜），故 3 人局恒为 1 轮或
  （无人淘汰时的）stalemate，不存在「需要第 2 轮投票」的路径。有博弈纵深的最小人数是 4。
...
判定（_judge_survivors，唯一判据来源）
├─ 卧底全出局 → civilians 胜，结束
├─ 卧底 >= 平民 → spy 胜，结束（人数打平即终局）
└─ 否则继续（0 < 卧底 < 平民），直至 max_rounds 上限 → stalemate
```
