...
> 当前验证版本：Iris 3.40.0 · v3.40.0 补强 chunk / 向量索引乐观并发控制与统一发布锁；v3.28.0 引入稳定 inode 锁、统一原子写、向量索引 generation 发布与跨进程 LLM 缓存治理。
...
| 步骤 | 内容 | 状态 |
|:---:|------|:---:|
| 1 | RMW 模式 FileLock 加锁（6 文件） | ✅ |
| 2 | SQLite WAL 模式 + 重试（1 文件） | ✅ |
| 3 | 双周报缓存加锁（2 文件） | ✅ |
| 4 | Agent 记忆隔离 IRIS_AGENT_ID（3 文件） | ✅ |
| 5 | 日志归档 TOCTOU 修复（1 文件） | ✅ |
| 6 | 进程注册表 ProcessRegistry（3 文件） | ✅ |
| 7 | 追加写 fcntl 保护（1 文件） | ✅ |
...
| 锁文件长期保留 | 每个受保护资源增加一个很小的 `.lock` 文件 | 这是正确性要求：释放后只解锁并关闭 fd，严禁 unlink |
...
- 所有 agent 共享同一份 LLM 响应缓存（内容寻址 + `FileLock`，访问时间与 LRU/TTL 元数据跨进程一致）
...
- 向量索引以 generation 目录完整写入，再原子切换 `current` 指针；读取方只消费完整代际
...
- **正确做法**：使用 generation 目录写入全部文件后原子切换指针（向量索引已采用），或引入 SQLite 存储图数据

### 锁文件生命周期

- `FileLock` 锁定的是 inode，不是路径字符串。释放时删除 `.lock` 文件会让等待者继续持有旧 inode，同时后来者创建并锁住新 inode，两个进程因此可能同时进入临界区。
- 正确生命周期是：创建一次、长期保留；释放只执行 `LOCK_UN` 和关闭 fd。`.lock` 文件是运行时协调制品，不是需要清理的临时文件。
