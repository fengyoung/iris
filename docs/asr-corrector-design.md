...
> 关联项目：Iris 3.24.2 / VocoType (AltRight 热键 + vocotype ASR)
> 状态：v3.24.2 真机修正 — full 模式一次写回 + 全场景逐字符 Delete（取消 Cmd+A）。v3.24.0 写回机制重构+LLM 相似度门槛+长度上限配置化。v3.23.3 双段流水线/短段门控。v3.22.5 热键门控修复。v3.19.12 上下文 A/B 对比。v3.19.10 ASR 引擎全面质量加固已完成
