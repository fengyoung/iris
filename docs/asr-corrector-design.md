...
> 关联项目：Iris 3.40.1 / VocoType (AltRight 热键 + vocotype ASR)
> 状态：v3.28.1 — LLM 深度校正输出不再被思考文本污染（provider 层 content 为空直接抛错走降级链，原静默回退思考过程当校正结果）；失败时自然落入「仅词典校正」降级路径。v3.24.3 全面优化（公共 push_context API + 结构化日志）。v3.24.2 真机修正 — full 模式一次写回 + 全场景逐字符 Delete（取消 Cmd+A）。v3.24.0 写回机制重构+LLM 相似度门槛+长度上限配置化。v3.23.3 双段流水线/短段门控。v3.22.5 热键门控修复。v3.19.12 上下文 A/B 对比。v3.19.10 ASR 引擎全面质量加固已完成
...
| 禁止项 | 无 | 不生成含括号注释的误识别（如"张三(误为张叁)"）；不生成原词大小写变体（qwen→QWEN 是格式归一，不是 ASR 误识） |
