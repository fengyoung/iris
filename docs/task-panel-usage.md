...
> 当前验证版本：Iris 3.40.1 · 功能基线：v3.27.0（v3.35.0 安全增补）。Web 只读展示层：在浏览器中查看 iris 任务状态与进程。操作仍在 CC CLI——面板不做任何任务操作（无 kill/重试），只回答「什么在跑、跑到哪了、出过什么事」。
...
任务记录默认只保存固定任务名。自定义命令摘要会在写入前脱敏 token、密钥、密码、授权和 prompt 参数，并截断至 240 字符；请勿把面板历史作为正文或凭证存储。
