...
- 最终全量 **3,621 通过**，覆盖率 **70.01%**；关键模块覆盖率、Ruff、mypy（192 源文件）、AST 安全扫描通过；桌面 1440px / 窄屏 500px 页面各 **26 项断言通过**；wheel、SPDX SBOM 与 macOS CLI smoke 成功。
- 环境限制：共享 Python 环境 `pip-audit` 返回非零（17 个包、119 条漏洞记录），含低于仓库下界的 requests / pytest / setuptools；不宣称发布门禁全部通过。Linux CI、Docker、真实模型联网对局未执行。完整证据和恢复边界见 [验收报告](docs/undercover-beta-acceptance-20260917.md)。本次保留未提交工作区，产品版本仍为 3.40.4。
