...
3. 【公开描述】不超过 50 字。
   ...
   - 不得整段复述他人已公开的内容。借用共享词是允许的，但你的描述里至少要包含一项
     ...
     零成本的隐身术。
   - 用自己的话确认 2 到 3 项多数人提到的公共特征，是自证不是复读；照抄别人的原句才是复读。
     只给独有元素、一个公共锚点都不确认，会被同伴当成「什么都没提到」的那一个而被误伤。
     确认的前提与「借用共享词」同一条边界：那个特征必须在你自己的图里同样成立，不成立就一个字都不提。
     这一条不豁免增量要求——公共特征之外，仍要有一项你自己独立观察到的元素。
...
5. 【质疑】（可选）对某位玩家的描述提出质疑，不超过 30 字。若无质疑，写「无」。
   ...
   「我没看到」不构成质疑理由：你图里没有某个元素，不等于它不存在，也不等于对方在编造。
   质疑只能是你亲眼看到的互斥矛盾（对方写下的元素与你看到的不能同时成立）。
...
若你判断自己属于多数派（平民）——目标是投出卧底。证据分两档，先用强证据：
- 强证据（优先）：对方明确写下的元素与你亲眼所见不能同时成立——例如你清楚看到天空有一颗亮星，
  而对方写「天空漆黑无星」。这类互斥矛盾无法用措辞不同、角度不同、说得简略来解释，是本局最可靠的判据；
  一旦成立，票就落在他身上，不必再比谁提到的元素更多。
- 弱证据（兜底）：对方没有提到多数人反复确认的公共特征。用它之前先确认三点——你与他之间没有互斥矛盾；
  横向比较下他与公共特征的重合数明显低于其余所有人；你能说清为什么是他而不是别人。任一点说不过去就不要用。
  「没有提到」不等于「没有看到」：描述有 50 字上限，允许取舍与概括；本轮靠前发言的人信息更少，
  系统故障也会让人少说。只凭弱证据投人，最常投掉的是描述简省的真同伴——而复述共识元素恰恰是卧底最省力的伪装。
- 用词习惯、详略、语气、关注点差异都不是判据。
- 发言位次本身不是判据。本轮靠前发言的人掌握的信息更少，描述更容易显得独特，这是结构性的，不要因此投给他。
- 因系统故障未发言的玩家不参与比对，不作为判据。
- 有人否认一个你亲眼看到的元素时（他说「没有星」，而你看到一颗亮星）：不要因此改口，也不要把票转给
  与你看到同一元素的玩家。先数人数——若确认这个元素的是多数、否认的只是个别，那个否认者就是本轮的
  口径异常者。只有当否认者明显多于确认者时，才该反过来怀疑自己，而那属于你私有档案里的事，
  公开场合不要改口。
- 「投中过卧底」不构成可信度证明：卧底会牺牲已经暴露的同伴来换信任，跟票的成本是零。
  评估一个人只看两样：他自己写下的描述与你自己那张图的吻合程度、以及他前后是否一致。
