...
> 使用方法与恢复边界见 [Web 使用说明](docs/undercover-web-ui.md)，本轮验收证据见 [验收记录](docs/undercover-beta-acceptance-20260917.md)。
...
- ✅ **断点真正接通**：v3.40.1 建的恢复通道此前只记轮号，且引擎从未消费它——恢复出的对局缺历史上下文。现改为保存完整轮次记录（公开描述、私有档案、投票及重投记录），引擎实际接入，从下一轮继续，并还原身份、存活名单与发言基准顺序
- ✅ **加版本闸**：新增 `resume_version=1`，拒绝只有轮号的旧断点及历史字段缺失、轮次不连续、玩家集合不合法的断点；读取非对象 JSON 按损坏断点处理（旧断点提示「请开始新局」而非静默恢复出错局）
- ✅ **消除恢复竞态**：原「先查并发槽位、后加锁注册」在双击时会起两个线程；现检查与注册同处 `games_lock` 内，重复恢复返回 409。校验失败不消耗对局名额，线程启动失败释放名额
- ✅ **保留手动模式**：`auto_advance` 随对局持久化，恢复后先进入轮间等待，支持推进与取消；页面「当前对局」在服务重启后先查复盘、再调恢复接口，并处理另一标签页抢先恢复
- ✅ 页面不再把恢复玩家**全部重置为存活**；种子校验与提交统一用 `Number`，修掉 `1e3` 校验通过却发送成 `1` 的不一致
- ✅ **轮次来源去重**：原先 `web_server` 另建一份累积列表与引擎事件各记一份，现只认事件 payload，消除两处状态可能不一致的隐患
- ✅ 全量 3,609 → **3,621 通过**，覆盖率 **70.00%**；桌面 1440px / 窄屏 500px 页面各 **26 项断言通过**；协议版本 3.24（不变）

**上一版 v3.40.4 (2026-09-16)** - 谁是卧底：胜负条件改为严格多数 + 投票判据按复盘证据分级：
...
- 3,621 个测试用例（pytest 全量），覆盖率 70.00%；Ruff、mypy（含显式兼容边界）、AST 安全扫描与 SPDX SBOM 门禁通过
...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v3.40.5** | 2026-09-17 | **谁是卧底断点恢复补完 + 最终验收**：v3.40.1 建立的恢复通道**其实没接通**——`checkpoint.json` 只记轮号，`_run_game_impl` 里那份 `_completed_rounds_for_ckpt` 也从未被引擎消费，恢复出的对局缺历史上下文。① **断点保存完整轮次记录**（公开描述、私有档案、投票及重投记录，`RoundRecord` 全量），引擎实际接入 `UndercoverGame.run()`，从下一轮继续，并还原身份、存活名单与发言基准顺序；终局断点直接结算、不再调用模型。② **加版本闸**：新增 `resume_version=1`；缺失该字段（仅轮号旧断点）、历史字段缺失、轮次不连续、玩家集合不合法一律拒绝，读取非对象 JSON 按损坏断点处理——旧断点提示「请开始新局」而非静默恢复出一局身份/存活错乱的对局。③ **消除恢复竞态**：原实现「先 `game_slots.acquire()` 查并发、再 `with games_lock` 注册」两步之间存在窗口，双击恢复会起两个线程跑同一 `game_id`；现检查仍在运行的会话与注册同处 `games_lock` 临界区内，重复恢复返回 409。校验全部在申请并发槽位**之前**完成（失败不消耗名额），`t.start()` 抛 `RuntimeError` 时回滚 `state.games` 条目并 `release()` 槽位。④ **保留手动模式**：`auto_advance` 写进 `setup` 随对局持久化，恢复后不再一律按自动续跑；手动模式先进入轮间等待，支持推进与取消，沿用 300 秒超时。⑤ **轮次来源去重**：原先 `web_server` 自建累积列表与引擎 `checkpoint` 事件各记一份轮次，现只认事件 payload 里的 `completed_rounds`。⑥ **页面修复**：恢复后不再把玩家**全部重置为存活**（改为读断点的存活名单）；种子校验与提交统一用 `Number`，修掉 `1e3` 能通过校验却被发成 `1` 的不一致；「当前对局」在服务重启后先查复盘、再调恢复接口，并处理另一标签页抢先恢复。⑦ **测试修正**：原全量失败源于新增用例替换了整个 `threading.Thread`，连真实 HTTP 请求线程一起拦截；现只替换游戏入口。补引擎终局/取消/残缺历史、HTTP 恢复接线与复盘落盘、真实 Chrome DOM 交互回归。全量 3,609 → **3,621 通过**，覆盖率 69.63% → **70.00%**；桌面 1440px / 窄屏 500px 页面各 26 项断言通过；wheel、SPDX SBOM 与 macOS CLI smoke 成功。**局限**：共享环境 `pip-audit` 未通过（17 个包、119 条漏洞记录，含低于仓库下界的 requests / pytest / setuptools），Linux CI、Docker、真实模型联网对局未执行——不宣称发布门禁全部通过。协议版本 3.24（不变）；数据版本与对局 `schema_version` 不变（`resume_version` 属本地恢复元数据，不进复盘格式） |
