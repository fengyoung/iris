...
**v3.11.7** — analysis/service.py 职责拆分重构：数据层/缓存层独立为独立模块，测试 277→315。
...
| **v3.11.5** | 2026-07-07 | 代码审查优化：12 项修复（UTC 时区/preview 注入/OP 缓存/ModelManagerError/reasoning fallback 等），219 测试 |
...
| **v3.11.0** | 2026-07-03 | Claude Code Skill 体系：6 个项目级 Skill（Wiki 管道 / 飞书导入 / 会议纪要 / 智能问答 / 报告生成 / 健康检查），169 测试 |
| **v3.10.2** | 2026-07-02 | feishu-doc-convert 改进：文件名使用飞书创建时间/作者，路由关键词调整，169 测试 |
| **v3.10.0** | 2026-07-01 | 全面代码优化 + 新模块引入（记忆 5 子模块 / 输出格式化 / 全局常量），4 项致命修复，169 测试 |
...
| **v3.11.0** | 2026-07-03 | Claude Code Skill 体系：6 个项目级 Skill（Wiki 管道 / 飞书导入 / 会议纪要 / 智能问答 / 报告生成 / 健康检查），169 测试 |
| **v3.10.2** | 2026-07-02 | feishu-doc-convert 改进：文件名使用飞书创建时间/作者，路由关键词调整，169 测试 |
| **v3.10.1** | 2026-07-02 | 版本号更新（3.10.0 → 3.10.1），169 测试 |
| **v3.10.0** | 2026-07-01 | 全面代码优化 + 新模块引入（记忆 5 子模块 / 输出格式化 / 全局常量），4 项致命修复，10 项 High 修复，169 测试 |
| **v3.9.0** | 2026-06-30 | 人物 Wiki 飞书通讯录丰富 + 人物发现规则增强（8 条新模式 + 排除名单），169 测试 |
