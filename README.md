...
**v3.40.6 (2026-09-17)** - 谁是卧底：卧底获胜条件改回「多数」：
- ✅ **多数即胜**：卧底人数 ≥ 平民人数即卧底获胜，**人数打平当场收局**（回滚 v3.40.4 的严格多数）。按试玩反馈，打平继续的体验不理想
- ✅ **判词收敛为单点来源**：同一条规则此前写在三处（主循环 / checkpoint 恢复分支 / `continuing` 互补式），现在统一走新增的 `_judge_survivors()`，「游戏继续 ⟺ 判词为空」由注释纪律变成结构保证
- ✅ **连动方向与上次相反**：`continuing` 从 `卧底 ≤ 平民` 改回 `卧底 < 平民`。方向反了的表现是**多发**一次 `round_waiting`——界面为已结束的对局亮着步进按钮，点一次才看到结果
- ✅ **提示词两侧同步**：描述与投票 prompt 都告诉模型「游戏继续 ⇒ 卧底少于平民」，只改引擎会让模型按半旧半新的规则推理；顺带补上投票侧此前**完全没有**的规则文本覆盖
- ✅ **已知代价**：3 人局现在恒为 1 轮——第 1 轮无论淘汰谁都直达终局（淘汰平民则 1v1 打平判卧底胜），有博弈纵深的最小人数是 4
- ✅ 全量 3,621 → **3,627 通过**（+6）；关键用例均实测确认**在旧代码下失败**，无反假阳性；协议版本 3.24（不变）

**上一版 v3.40.5 (2026-09-17)** - 谁是卧底断点恢复补完 + 最终验收：
...
- 3,627 个测试用例（pytest 全量），覆盖率 69.99%；Ruff、mypy（含显式兼容边界）、AST 安全扫描与 SPDX SBOM 门禁通过
...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v3.40.6** | 2026-09-17 | **谁是卧底：卧底获胜条件改回「多数」**（回滚 v3.40.4 的严格多数）：按试玩反馈，打平继续的体验不理想，改回通行玩法——**卧底人数 ≥ 平民人数即卧底获胜**，人数打平当场收局。① **判据收敛为单点来源**：同一条规则此前写在**三处**（主循环 `spy_alive > civ_alive`、`run()` 的 checkpoint 恢复分支、手动步进的 `continuing` 互补式），规则一改就要人工同步三处，而漏改恰是**没有失败信号**的那一类（v3.40.4 修过一次的同型问题）；现新增 `_judge_survivors()` 作为唯一判据来源，主循环与恢复分支都调它，`continuing` 直接取反 `not verdict`，顺带消掉 `alive`/`survivors` 两份重复计算。② **连动方向与上次相反**：`continuing` 从 `卧底 ≤ 平民` 改回 `<`；方向反了的表现是**多发**一次 `round_waiting`——打平轮已判胜负，界面却让玩家为一场已结束的对局点一次步进。守卫用例同时断言事件列表、`winner` 与轮数，因为 `[]` 是「坏了也成立」的值（事件管道整体失灵也能满足）。③ **提示词两侧同步**：规则文本在 `_DESCRIBE_PROMPT` 与 `_VOTE_PROMPT` 各有一处，且都向玩家陈述了可推断的事实（「游戏继续 ⇒ 卧底人数**不多于**平民」），只改引擎会让模型按半旧半新的规则推理；两处一并改为「少于」。另发现 `_VOTE_PROMPT` 此前**从未写过胜负条件**、只有这句推断，已把两侧规则文本一并钉进测试。④ **已知代价不掩饰**：3 人局博弈纵深归零——1 卧底 + 2 平民时第 1 轮无论淘汰谁都直达终局（淘汰平民则 1v1 打平判卧底胜，淘汰卧底则卧底清零判平民胜），故 3 人局恒为 1 轮或 stalemate，有博弈纵深的最小人数是 4。⑤ **反假阳性**：每条关键用例均实测确认**在旧代码下失败**——引擎改动 `git stash` 后单跑，5 条编排级用例全挂（含新加的 checkpoint 打平档，它是恢复路径那份判词副本的唯一守卫，旧参数恰好避开打平）；7 条 prompt 契约断言逐条比对旧文本，全部不成立。删 1 条已不可构造的用例（打平后翻盘路径不存在了），全量 3,621 → **3,627 通过**（+6），覆盖率 70.00% → 69.99%；协议版本 3.24（不变） |
