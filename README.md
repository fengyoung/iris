...
**v3.18.1** — 全栈代码质量优化：异常处理加固、性能优化、ConfigBundle 统一 Pydantic v2、核心测试 +42。Wiki 192 页 / 知识图谱 192 节点 778 边 / 数据源 703 文档 / 4,096 Chunk / 向量索引 7,035 条。
...
- 1,265 个单元测试（73 个测试文件），覆盖率 53%（仅统计 Iris 自身 LLM 调用）
...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v3.18.1** | 2026-07-16 | 全栈代码质量优化：P0 异常处理加固（15文件）+ P1 共享线程池/ConfigBundle Pydantic v2/deep_eval拆分 + P2 内存LRU缓存/BM25统计缓存 + P3 飞书退避优化/警告过滤；测试 1223→1265 | 
