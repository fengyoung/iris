...
**v3.19.10** — ASR 引擎全面质量加固（P0~P3 十四项）：P0 Prompt 字符串截断修复 / P1 非修饰键校验 + 热词超时 + 参数化 / P2 预检查 + warning + 校验 / P3 Aho-Corasick 优化 + worker 动态 + 单字符代码分档 + 重试（8 文件，+313 / -114 行）。
...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v3.19.10** | 2026-07-21 | ASR 引擎全面质量加固（P0~P3）：P0 protected_terms 截断 / P1 热键校验+超时+参数化 / P2 预检查+warning+校验 / P3 Aho-Corasick 优化+worker 动态+代码分档+重试（8 文件，+313/-114 行） |
