...
**v3.19.20** — ASR 反馈反向优化引擎：feedback.jsonl 驱动词典自动进化（僵尸规则检测 / LLM 发现提升为词典 / 热词补充），build-asr-prompt 集成自动优化。新增 `test_asr_feedback.py` 31 用例。v3.19.19 测试覆盖全面优化：P1 三模块接近全绿（97%/98%/70%），新增 3 测试文件 +90 用例（1,858→1,948），覆盖率 58.17%→59%。
