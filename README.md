...
**v3.18.4** — 代码质量优化：正确性修复（`_rrf_fuse` 逻辑 bug + 裸 except）+ 技术债清理（rrf 配置统一 + shared_pool 接入）+ graph.py 拆分（973→747 行）+ 测试补充（1,467 通过，llm/service 97%，memory/session 100%）。Wiki 192 页 / 知识图谱 192 节点 778 边 / 数据源 703 文档 / 4,096 Chunk / 向量索引 7,035 条。
