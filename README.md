...
**v3.29.2** — [提醒引擎项目停滞误判修复](CHANGELOG.md)：拍照3.0 等含版本号项目 slug 去点且丢失词边界，导致活跃项目被误判停滞（138 天）。修复：①数字点号变体还原（30→3.0，排除年份）；②读取项目页 frontmatter title 按空格切分补充词段关键词（拍照3.0 / AI外观定级），命中会议纪要/简报等近期活动文档；③调薪完结项目入 `project_stall_ignore`。停滞信号 6→1。协议版本 3.21（不变）。
