...
**v3.19.18** — 知识库质量全面加固：wiki-pipeline 已有页面检测修复（0%→85%）、知识图谱 LLM 语义关系提取（+986 边）、断链清零（11→0）、零出链清零（27→0）、向量索引构建（9,019 条）、LLM 用量追踪体系完善（embedding 纳入 + CLI/Skill 来源标记）。13 源文件 + 9 Skill 文件。v3.19.17 SOURCE 目录按月/年归档；多 Agent 并发安全。全量 1,858 测试通过。
**v3.19.19** — 测试覆盖全面优化：P1 `_graph_engine` 61%→97%（纯 Python 回退路径全覆盖）、`asr/formatter` 59%→98%（热词/词典/Prompt 渲染）、`navigation` 67%→70%（断裂链接 7 豁免分支 + changelog/原子写入）；P2 `generator` 纯逻辑增强（内容提取/更新校验/引用质量）。新增 3 个测试文件 + 90 用例（1,858→1,948），总覆盖率 58.17%→59%。

**v3.19.18** — 代码质量全面加固：P0 静默异常修复（session.py 迁移失败加日志）+ P1 DRY 消除（llm 缓存检查 / service 死委托 / generator prompt 重复）+ P2 工程增强（pip-audit / constraints.txt / git tag 发布流程）+ P3 测试精化（14 文件正确分类，单元测试 526→794）+ lint 清零。11 文件，+176 / −147 行。全量 1,858 测试通过。
...
| 版本 | 日期 | 要点 |
|------|------|------|
| **v3.19.18** | 2026-07-23 | 知识库质量全面加固：wiki-pipeline 检测修复 + LLM 语义关系提取(986边) + 断链/零出链双清零 + 向量索引构建(9,019) + 用量追踪 CLI/Skill 分离 |
...
| **v3.19.18** | 2026-07-23 | 代码质量全面加固：P0 静默异常修复 + P1 DRY 消除（缓存/委托/prompt）+ P2 工程增强（pip-audit/constraints/git-tag）+ P3 测试分类精化（794 unit） + lint 清零 |
