...
> 2026-09-17 工作区待发布优化：谁是卧底断点保存完整 `RoundRecord`，以 `resume_version=1` 区分仅轮号旧断点；恢复保持身份、淘汰状态与手动模式。HTTP 恢复检查和会话注册共用 `games_lock`，避免重复线程。页面“当前对局”接通服务重启后的恢复接口。验收与边界见 [本轮报告](docs/undercover-beta-acceptance-20260917.md)。产品版本尚未发布递增，协议和配置版本不变。
...
**当前规模**：~45,000 行 / 192 个源码文件 / 28 模块 · CLI 70 命令 · 测试 3,609（pytest 全量）· 覆盖率 69.63%（`fail_under` 65）· Ruff、mypy（含显式兼容边界）、AST 安全扫描与 SPDX SBOM 门禁通过（动态/平台专属模块设显式兼容边界）· 10 个项目级 Skill · Wiki 238 页 · 知识图谱节点 238 / 关系边 2,724（wikilink 1,115 + LLM 1,609）· 数据源 900+ 文档 / 6,771 Chunk（text-embedding-v3 / 1,024 维）

本版把「谁是卧底」的胜负条件改为**严格多数**（卧底人数 > 平民人数才获胜，打平继续），并按上一局复盘的票理由证据校准投票判据——**主因是平民大批出局不是因为暴露，而是因为说得少**：那局出局的 7 人中 6 名是平民、票理由几乎全是「未提及型」，而唯一出局的卧底死于明确互斥矛盾，说明旧判据在惩罚发言简省而非识别身份差异。另有一处必须同步改的隐性连动（手动步进的 `continuing` 判定，漏改则打平轮不发 `round_waiting`、界面不报错但步进被静默跳过）。详见 CHANGELOG。

上一版 v3.40.3 修「谁是卧底」的三处**静默失效**（功能照跑、界面不报错，只是结果不对）：合并遗留的 DOM 悬空引用使网页端开不了局；总结模型未接线使总结从未生成；投票解析器的整段兜底按 key 长度挑目标，使投票理由与投票对象对不上。另按一局真实复盘的证据校准了游戏策略 prompt。
...
| 层 | 位置 | 当前值 | 含义 |
|------|------|:---:|------|
| **产品版本** | `pyproject.toml` | 3.40.4 | 软件发布版本 |
