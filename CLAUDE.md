...
**当前规模**：~43,000 行 / 185 个源码文件 / 27 模块 · CLI 68 命令 · 测试 3,317（pytest 全量）· 覆盖率约 68%（`fail_under` 65）· Ruff 与严格 mypy 门禁通过（动态/平台专属模块设显式兼容边界）· 10 个项目级 Skill · Wiki 241 页 · 知识图谱节点 220 / 关系边 1,858（wikilink 1,225 + LLM 633）· 数据源 900+ 文档 / 6,771 Chunk（text-embedding-v3 / 1,024 维）
...
├── tests/             # 3,317 用例（pytest 全量，conftest 自动打标记）
