...
| 层 | 位置 | 格式 | 含义 | 当前值 |
|----|------|------|------|--------|
| **产品版本** | `pyproject.toml` | SemVer X.Y.Z | 软件发布版本 | 3.11.1 |
...
| # | 问题 | 修复 | 方法 |
|:---:|------|------|------|
| 1 | 日期错误 — header 和尾注都使用生成日期而非会议日期 | header 和尾注中的录音日期改为从文件名提取 | `_format_meeting_date()` |
| 2 | 缺少时长 — 纪要无 `时长：X小时X分` 字段 | 从转写文本时间戳（`HH:MM:SS`）计算首尾时差 | `_calc_duration()` |
| 3 | 尾注缺失 — 12/40 文件无 `生成说明` 尾注 | LLM 输出后检查，缺少则自动追加 | `_ensure_footer()` |
