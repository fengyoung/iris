...
**当前规模**：~45,000 行 / 190 个源码文件 / 28 模块 · CLI 70 命令 · 测试 3,502（pytest 全量）· 覆盖率约 69%（`fail_under` 65）· Ruff、严格 mypy、AST 安全扫描与 SPDX SBOM 门禁通过（动态/平台专属模块设显式兼容边界）· 10 个项目级 Skill · Wiki 238 页 · 知识图谱节点 238 / 关系边 2,724（wikilink 1,115 + LLM 1,609）· 数据源 900+ 文档 / 6,771 Chunk（text-embedding-v3 / 1,024 维）

本版本修一处**没有失败信号**的静默截断：CC 记忆文件的 frontmatter 里出现三连字符字面量时（典型是在 description 里举例 YAML 分隔符），该条记忆永远同步不到 Iris，而同步照跑、退出码 0。根因是 `sync_memory.py` 用 `text.split("---", 2)` 切 frontmatter——字段值里的三连字符被当成闭分隔符，其后的 `type` 等字段全部跌出解析范围，`type` 解析为空串、被 `_classify` 判为「无 type」而跳过。这个洞已真实发生两次：`wiki-lint-fix-bug.md` 首次踩坑留下 6 行残骸（截断尾部 + 有人试图补 `metadata: type:` 却补在了字面层的正文里），本次核验时同类写法再次复现，靠比对 `_parse_frontmatter` 的实际返回值才发现。修法是新增 `_FRONTMATTER_RE` 与 `_split_frontmatter()`，闭分隔符须**独立成行**才算数，`_parse_frontmatter` / `_extract_body` 统一走它；顺带修正 `---xyz` 与缩进三连字符两处更早的误判，清理 6 个死代码常量。

上一版 v3.39.1 修一处**没有失败信号**的错报：`generate_sbom.py` 归档出的 SBOM 把 iris 声明成 3.27.0，而源码树是 3.39.0。根因是「用安装状态代替源码事实」——脚本用 `importlib.metadata` 取全部包版本，这对第三方依赖是对的（`.dist-info` 由 pip 写入，与落盘代码一致），但对 iris 自身是错的：editable 安装的 `.dist-info` 冻结在**上次重装那一刻**，改源码、升版本、提交、推送都不刷新它。SBOM 是供应链审计的输入，声明错版本意味着后续排查会对错版本的代码，而它照常生成、结构合法、退出码 0，只有人工比对 `pyproject.toml` 才看得出。修法是 iris 自身改读 `pyproject.toml`（第三方仍读已安装元数据，各取其可信来源），读不到版本时返回 1 且**不写出文件**——宁可不生成，也不要生成一份说谎的发布制品。
...
| 层 | 位置 | 当前值 | 含义 |
|------|------|:---:|------|
| **产品版本** | `pyproject.toml` | 3.39.2 | 软件发布版本 |
| **协议版本** | `src/iris/__init__.py` | 3.24 | CLI 命令集 / agent-spec 格式 |
