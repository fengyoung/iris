...
**当前规模**：~42,000 行 / 185 个源码文件 / 27 模块 · CLI 68 命令 · 测试 3,350（pytest 全量）· 覆盖率约 68%（`fail_under` 65）· Ruff、严格 mypy、AST 安全扫描与 SPDX SBOM 门禁通过（动态/平台专属模块设显式兼容边界）· 10 个项目级 Skill · Wiki 238 页 · 知识图谱节点 238 / 关系边 2,724（wikilink 1,115 + LLM 1,609）· 数据源 900+ 文档 / 6,771 Chunk（text-embedding-v3 / 1,024 维）
...
├── tests/             # 3,350 用例（pytest 全量，conftest 自动打标记）
