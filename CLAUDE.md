...
| 层 | 位置 | 当前值 | 含义 |
|------|------|:---:|------|
| **产品版本** | `pyproject.toml` | 3.18.2 | 软件发布版本 |
...
**当前 v3.18.2 (2026-07-16)** — 全栈代码质量优化。① 异常处理：消除所有裸 pass 吞异常（15 文件）。② 性能：共享线程池 + 内存 LRU 缓存 + BM25 统计缓存。③ 架构：ConfigBundle 统一 Pydantic v2 + deep_eval 拆分 + 核心模块测试 +68。④ 工程：飞书退避 2^n+抖动、警告过滤、字段默认值完善。测试 1,291 通过。
